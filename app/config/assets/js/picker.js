...
$(function () {
  ...
  function watchForm(params)
  {
    ...
    if (f[0].checkValidity() === true) {
      ...
      if (Number(params.status) === 6) params.form_def = 'remeasure'; // edit this to mortality if you want that kinda thing
...
function queryDB(table, query, params) {
  ...
  var successFn = function (result) {
    ...
    for (var row = 0; row < result.getCount(); row++) {
      ...
      r['northing']   = result.getData(row, "Northing");
      r['easting']    = result.getData(row, "Easting");
      ...
      r['height']     = result.getData(row, "PrevHeight");
